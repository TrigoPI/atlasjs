...
describe("Scheduler — legacy facade", () => {
  it("orders legacy steps by numeric priority", () => {
    const s = new Scheduler();
    const t = trace();

    s.onUpdate(t.push("late"), { name: "late", priority: 800 });
    s.onUpdate(t.push("early"), { name: "early", priority: 100 });
    s.onUpdate(t.push("mid"), { name: "mid", priority: 300 });

    s.runUpdate(1 / 60);

    expect(t.log).toEqual(["early", "mid", "late"]);
  });

  it("passes dt to legacy callbacks", () => {
    const s = new Scheduler();
    let seen = -1;

    s.onFixedUpdate((dt) => (seen = dt), { name: "capture", priority: 100 });
    s.runFixedUpdate(0.5);

    expect(seen).toBe(0.5);
  });

  it("interleaves legacy steps with staged steps on the shared axis", () => {
    const s = new Scheduler();
    const t = trace();

    // Staged step in PhysicsStep (anchor 300) and legacy steps around it.
    s.fixed.add(t.push("staged-step"), {
      name: "staged-step",
      stage: "PhysicsStep",
    });
    s.onFixedUpdate(t.push("legacy-100"), {
      name: "legacy-100",
      priority: 100,
    });
    s.onFixedUpdate(t.push("legacy-900"), {
      name: "legacy-900",
      priority: 900,
    });
    ...
    expect(t.log).toEqual(["legacy-100", "staged-step", "legacy-900"]);
  });
});
