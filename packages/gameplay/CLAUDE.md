...
> - `docs/gameplay/scripting-components.md` — the Unity-`GetComponent` model, the `defineScriptComponent` token model, the façade⇔real-behavior rule below, and the clean-alias vocabulary (history appendix records the earlier class-façade + unification steps).
