...
type AssetAtlas<T extends string = string> = Record<T, Resource>;

export class AssetsLoader<T extends string> {
  ...
  public getAsset<K extends Resource>(name: T): K {
  ...
  public addAudio(name: T, audio: string): AssetsLoader<T> {
    this.audios.push({ name, asset: new AudioClipAsset(audio) });
    return this;
  }

  public addTexture(name: T, texture: string): AssetsLoader<T> {
    this.textures.push({ name, asset: new TextureAsset(texture) });
    return this;
  }

  public addSprite(name: T, sprite: string): AssetsLoader<T> {
