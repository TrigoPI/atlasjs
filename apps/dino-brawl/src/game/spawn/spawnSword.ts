...
export async function spawnSword(
  ctx: SceneContext,
  owner: Entity,
): Promise<void> {
  ...
  const swordSpriteAsset: SpriteAsset = SpriteAsset.fromPath(
    ResourcesPath.Sprites.Swords.Default,
  );
  ...
  const swordRender: SpriteRenderer = nexus.addComponent(
    sword,
    SpriteRenderer,
    swordSprite,
  );
