...
import type {
  ...
  PointObject,
  TileObject,
} from "../../src/game/tiled/resolved.types";
  ...
  tilesets: [
    {
      ...
      tilewidth: 32,
      tileheight: 32,
      image: "../assets/tilesets/ground/grass-tileset.png",
    },
    {
      name: "props",
      firstgid: 65,
...
describe("TiledDocument tilesets", () => {
  it("keeps only tilesets that have an image", () => {
    ...
    expect(doc.tilesets[0]).toMatchObject({
      firstGid: 1,
      columns: 8,
      rows: 8,
      tileCount: 64,
...
describe("TiledDocument tile layers", () => {
  it("flattens groups and records the group path + stacking order", () => {
    ...
    expect(doc.tileLayers[0]).toMatchObject({
      name: "ground_layer",
      groupPath: ["ground"],
      order: 0,
    });
...
describe("TiledDocument objects", () => {
  it("types point / tile / rect objects and carries the group path", () => {
    const doc = new TiledDocument(fixture);
    const byName: Record<string, unknown> = Object.fromEntries(
      doc.objects.map((o) => [o.name, o]),
    );
    ...
    expect(spawn).toMatchObject({
      ...
      x: 48,
      y: 32,
      groupPath: ["systems", "systems_objects"],
    });
