...
### E3. Dispatch de node câblé à 6 endroits + duplication renderers — ✅ fait

- **Constat** : ajouter un kind (texte **A2**, particules) imposait d'éditer 6 sites hardcodés à 2 kinds : l'union `DrawCommand`, le `instanceof` de `SceneRenderer.collect`, la construction des batchers, le `if/else` de `RenderQueue.flush`, les deux `Batcher` structurellement identiques, et `SpriteRenderer`/`ShapeRenderer` qui dupliquaient `RENDER_STATES` (octet pour octet), les constantes de z-packing, `computeSortKey` et le pattern `getOrCreateRenderData` (WeakMap).
- **Livré** : seam `NodeRenderer { kind; matches(node); collect(node, viewport, scratch); createBatcher(renderer) }` (`renderers/NodeRenderer.ts`, avec l'interface `Batcher`) + registre `Map<kind, Batcher>` dans `RenderQueue` (`register`/`flush(renderer)` générique, plus de `if/else`) + `NodeRendererBase<TNode, TData>` partagé (`RENDER_STATES`, z-packing, `computeSortKey`, `getOrCreateRenderData`). `SceneRenderer.collect` est une boucle unique (itère les `NodeRenderer` enregistrés → `matches` → `collect` → `submit`) : plus de chaîne `instanceof`. `computeModelWorldBound` (`renderers/worldBound.ts`) factorise l'AABB monde. **`DrawCommand` gardé concret** (union taguée). **Culling par-renderer dans `collect`** pour préserver l'ordre exact : sprite cull-avant-build (le `getBatchId` premier-vu ne tourne que pour les sprites visibles → z-order inchangé — bug de régression z-order attrapé en review et corrigé), shape build-puis-cull (batchKey statique, sans effet de bord). Ajouter un kind = un `NodeRenderer` + une entrée dans `SceneRenderer.nodeRenderers` + un membre concret de `DrawCommand` (de 6 sites à ~3 ; `flush`/`collect`/wiring batcher plus jamais touchés). Verrouillé par `packages/nebula/test/{worldBound,NodeRendererBase,RenderQueue}.test.ts` (logique pure). Vérif runtime : `apps/webgpu` rend identique (sprite/rect/cercle/ligne), zéro erreur.
- **Fichiers** : `packages/nebula/src/renderers/{NodeRenderer,NodeRendererBase,worldBound,SceneRenderer,RenderQueue,Batchers,SpriteRenderer,ShapeRenderer,index}.ts`, tests `packages/nebula/test/{worldBound,NodeRendererBase,RenderQueue}.test.ts`.
