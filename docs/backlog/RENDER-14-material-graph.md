...
Aucune brique de cette vision n'existe dans le code : ni `MaterialData`, ni `materialize`/`dematerialize`, ni package `@atlasjs/material-graph`, ni registre de nœuds WGSL (confirmé sur `packages/nebula/src/core/material/Material.ts`, qui ne connaît que l'objet `Material` vivant, et par l'absence de tout dossier `material-graph` hors `docs/`). Le design est entier et cadré (`docs/rendering/material-graph.md`) mais reste une seule vision indivisible aujourd'hui : chaque brique dépend de la précédente et aucune n'a de début d'implémentation distinct à isoler en tâche autonome. La découper en plusieurs notes suggérerait un séquencement qui n'existe pas encore.
