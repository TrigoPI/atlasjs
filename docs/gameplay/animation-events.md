...
> **Statut : implémenté.** Ajoute trois events au composant `Animator` : `started`, `finished`, `loop`. Extension incrémentale de `docs/gameplay/sprite-animation.md` (sort `onComplete` de sa liste V2 §10).
> Prérequis de lecture : `docs/gameplay/sprite-animation.md` (le composant `Animator` + `AnimatorSystem`), `packages/core` `EventBus` (pub/sub typé réutilisé ici).
