...
> **Statut : implémenté** (`defineActions`, builders `button()`/`vector2()`, `InputActionMap`, `PlayerInput` + `PlayerInputSystem` — voir `packages/input/src/public/actions/` + `packages/gameplay/src/components/PlayerInput.ts` + tests `actions.test.ts`, `action-runtime.test.ts`, `player-input.test.ts` ; utilisé dans `apps/sandbox` `TestScript.ts`). Phase 2 de l'input dans le scripting, suite de `docs/gameplay/input-scripting.md` (Phase 1 = polling bas-niveau via façade `InputApi`, livrée). Objectif : une abstraction haut-niveau façon [Unity Input System](https://docs.unity3d.com/Packages/com.unity.inputsystem@1.19/manual/QuickStartGuide.html) — des **actions nommées** (`jump.isPressed()`, `move.readValue()`) plutôt que des touches en dur dans la logique de jeu.
>
