...
- 📋 **Gizmos d'éditeur** : poignées de sélection/déplacement/échelle, contour de l'entité sélectionnée, preview de collider en cours d'édition. Le seam est déjà posé — tout producteur s'insère avec `before: "gizmos:flush"` sans toucher au package. Un futur éditeur devrait être **producteur** de l'API immediate-mode plutôt que porter son propre chemin de rendu ; les deux briques qu'il réclamera sont déjà dans cette liste (passe overlay hors scene-graph, épaisseur constante à l'écran).
