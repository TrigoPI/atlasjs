...
`docs/` holds cross-cutting design documents and architecture decisions that span multiple packages, organized by domain (`rendering/`, `core/`, `gameplay/`, `assets/`, `debug/`). The up-to-date index is [`docs/README.md`](docs/README.md), regenerated by `pnpm docs:index` — do not hand-maintain a document list here.
