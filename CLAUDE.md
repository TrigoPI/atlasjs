...
`memory/atlas/` holds cross-cutting design documents and architecture decisions that span multiple packages, organized by domain (`rendering/`, `core/`, `gameplay/`, `assets/`, `debug/`, `physics/`). The entry point is [`memory/atlas/README.md`](memory/atlas/README.md), which points at the Obsidian Bases that index the vault — do not hand-maintain a document list here.

Consult it before undertaking a significant change: a design may already be validated and awaiting implementation. **Anything left to do lives in `memory/atlas/backlog/`** — one note per item with a status and last-verified date, viewed through [`memory/atlas/backlog.base`](memory/atlas/backlog.base). Check it first for pending work.
...
- Check `memory/atlas/` for an existing design document covering the change.
...
- **A green build is not a type-check: `tsdown build` never checks function bodies.** `pnpm --filter <pkg> build` can pass while `tsc --noEmit` on the same package fails — `@atlasjs/rapier` shipped a `TS2554` through a green build during the contact-point work. Rebuilding a `dist` proves it is fresh, not that it is correct; run the type-check too. Corollary: a package's `tsconfig.json` usually carries `"include": ["src"]`, so **`test/` is not type-checked at all** — `packages/gameplay` wires its `typecheck` script to a separate `tsconfig.test.json`, and that is what caught an error the main tsconfig ignored.
...
- Write design/spec documents directly into `memory/atlas/` (flat), not into a nested `memory/atlas/superpowers/specs/` folder.
