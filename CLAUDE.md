...
- `docs/gameplay-redesign.md` — validated redesign (design only, not implemented) of `gameplay` scripting/sync: kills the shadow-ECS + diff/request/feedback pipeline (4 copies of one datum) in favor of a **single source of truth** per datum in Nexus, a **thin façade** (Unity-style proxy, not a copy) for the script API with lazy resolution + event-invalidated cache, a physics bridge with **authority declared by body type** (dynamic → physics, kinematic/static → transform), and runtime handles (`PhysicsBodyRef`) as Nexus components. Collapses ~7 sync systems into 2. See the checklist at the bottom of the doc.
