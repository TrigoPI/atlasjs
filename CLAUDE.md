...
- `docs/gameplay/input-scripting.md` — **implemented** (Phase 1 of input scripting). Service access to scripts via high-level façades (generic `ScriptService<TService>`), delivers the `InputApi` façade (global keyboard/mouse polling). See `packages/gameplay/src/scripting/services/`.
...
- `docs/gameplay/sprite-animation.md` — **validated, not implemented**. Sprite animation wired into the gameplay path: `Animator` (LEVEL-1 component, named clips + `play(name)`) + `AnimatorSystem` (dt-driven, pushes the current frame into `SpriteRender.sprite` via the existing swap path, `Map<Frame, Sprite>` cache), reusing nebula `SpriteSheet`/`SpriteAnimation` (the latter made dt-driven). Next feature to implement (→ backlog).
