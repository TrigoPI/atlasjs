...
- **Command buffer** pour la mutation structurelle pendant une frame, avec **flush hybride** : auto aux sync points (fin de stage/lane, aligné sur `memory/atlas/core/scheduling.md`) + `world.flush()` manuel disponible.
...
- **Flush hybride** : le flush auto est câblé au niveau **core** — chaque lane se termine par une étape `Sync` (anchor 1000, cf. `memory/atlas/core/scheduling.md`), et `NexusPlugin` y enregistre un step `nexus:flush` dans les 3 lanes. Tout plugin utilisant le world en bénéficie **sans le câbler**. `world.flush()` reste disponible pour forcer manuellement dans un système précis.
