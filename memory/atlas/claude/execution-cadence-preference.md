...
**How to apply:** implementers implement + test but never commit; controller pauses after each task's review for the human to commit. Reply in French for superpowers-driven sessions (session started in French). See root [`CLAUDE.md`](../../../CLAUDE.md) rule "don't commit automatically" — this is the cadence that goes with it.
