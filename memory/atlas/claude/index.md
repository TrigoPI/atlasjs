...
- [[inertial-movement-feature]] — bump-royal en dynamique piloté par la vélocité (mergé dans dev le 2026-09-01, non poussé) ; `lockRotations` n'annule pas une angvel existante ; `RigidBody2D.mass` est bien autoritatif à densité 0 ; les deux apps divergent volontairement sur le mouvement
- [[cpu-particles-feature]] — système de particules CPU **mergé dans dev** (`bd0a802`, 2026-09-04, non poussé), vérifié navigateur ; réutilise le sprite batch donc nebula-webgpu intact ; le goulot est packStorageArray, pas la boucle de sim ; un sprite quasi vide en additif ressemble à un pipeline cassé
- [[gameplay-plugin-test-imports]] — un test plugin de gameplay doit importer depuis `../src/`, pas `@atlasjs/gameplay` : le nom du package donne une 2e identité de classe et les systèmes ne voient rien, sans erreur
- [[collision-contact-points-feature]] — le point de contact remonte aux scripts (mergé dans dev le 2026-09-06, non poussé) ; deux conventions de normale (`ContactPoint` : a→b ; `Collision` : other→soi), objet prêté que le `readonly` ne protège pas ; un build tsdown vert ne type-checke aucun corps de fonction ; une spec de conversion d'unités centrée à l'origine ne discrimine rien
- [[bump-royal-online-feature]] — bump-royal en ligne (mergé dans dev le 2026-09-07, non poussé) ; un Engine Atlas tourne headless en Node **sans travail moteur** (stub nebula, pas d'input, pas d'audio, surtout pas de caméra) ; ScriptManager désactive en silence un script dont `onCreate` throw
- [[mutation-test-gameplay-specs]] — trois specs vertes qui n'observaient rien en une seule série : toujours casser le code exprès et coller la sortie d'échec
