...
- [[collision-contact-points-feature]] — le point de contact remonte aux scripts (livré sur feat/claude/collision-contact-points, non mergé) ; deux conventions de normale (`ContactPoint` : a→b ; `Collision` : other→soi), objet prêté que le `readonly` ne protège pas ; un build tsdown vert ne type-checke aucun corps de fonction ; une spec de conversion d'unités centrée à l'origine ne discrimine rien
