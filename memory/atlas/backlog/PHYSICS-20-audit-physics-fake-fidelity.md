...
**Amorcée le 2026-09-05.** [[collision-contacts]] a livré `packages/rapier/test/collision-contacts.test.ts` — 9 specs de **résolution de contacts** contre le vrai moteur, en Node pur : espace de la normale, point sur la face, conversion d'unités, sensor et fin de collision. C'est le premier morceau réel de la suite rapier-only décrite ici, et il confirme le partage proposé : le faux rend `null` pour tout contact, par construction et à assumer. Deux enseignements réutilisables pour la suite de cette suite : une géométrie **alignée sur les axes** ne discrimine pas un repère local d'un repère monde (la spec de normale a dû tourner une boîte à 45°), et une scène **centrée à l'origine** ne discrimine pas une conversion d'unités absente (`0 × 100 = 0` — la scène a dû être décalée à 5 m / 500 unités).
