...
**Le précédent existe désormais, sur cette ligne exacte.** [[collision-contacts]] a livré (2026-09-05) un objet `Collision` **unique, détenu par le système et re-ciblé avant chaque dispatch**, avec le contrat « prêté, pas donné » écrit en JSDoc sur le type et sur le paramètre de `onCollisionEnter`. La première piste ci-dessous n'est donc plus une hypothèse à défendre de zéro : la même décision a été prise, argumentée par cette note-ci, et la seconde valeur passée à `onCollisionEnter` est déjà un objet prêté. Deux conséquences pratiques : le contrat à documenter est **déjà formulé** et n'aurait qu'à s'étendre au handle (« ce que `onCollisionEnter` reçoit ne survit pas au callback », les deux arguments compris), et le prix à connaître est celui que ce design a nommé — un `readonly` de TypeScript ne protège aucune mutation interne, seule la JSDoc défend la règle.
