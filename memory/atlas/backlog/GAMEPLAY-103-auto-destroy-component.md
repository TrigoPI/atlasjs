...
**À rapprocher de :** [[GAMEPLAY-96-script-timers]] — `afterSeconds` est le même besoin de planification temporelle, vu depuis le composant plutôt que depuis le script ; si un ordonnanceur de délais arrive côté `AtlasScript`, ce canal-là d'`AutoDestroy` doit s'appuyer dessus au lieu de compter son propre `dt`. Et [[GAMEPLAY-109-particle-system]] — un système de particules dédié rendrait `AutoDestroy` inutile pour les particules, mais pas pour les FX one-shot comme `Impact`.
