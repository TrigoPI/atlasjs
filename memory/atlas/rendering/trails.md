...
> Contexte : première brique de VFX du moteur. Suit le grain posé par `memory/atlas/rendering/renderer-architecture.md` et `memory/atlas/rendering/shapes.md` : un nouveau `NodeRenderer` branché sur la file de rendu commune, sans nouveau mécanisme de batching. Aucune forme n'était exposée au niveau ECS avant ce design — `TrailRenderer` est le premier renderer non-sprite côté `gameplay`.
...
À verser dans `memory/atlas/backlog/` à la clôture :
