...
Le **stroke** n'atterrit pas ici : c'est une capacité de rendu, pas du debug. Il va dans `@atlasjs/nebula` (+ backend `nebula-webgpu`) et ferme l'item backlog « strokes / contours » de `memory/atlas/rendering/shapes.md`.
...
1. **Aucun slot `params` brûlé.** `params.z/w` sont **réservés aux coins arrondis / feather** par `memory/atlas/rendering/shapes.md`. Les dériver de la matrice les laisse libres.
