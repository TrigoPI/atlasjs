...
> Prérequis de lecture : `memory/atlas/assets/asset-system.md` (le couple `Asset`/`Resource` + `AssetManager` + loaders par type, calqué ici), `memory/atlas/gameplay/sprite-animation.md` (le couple composant `Animator` + `AnimatorSystem`, calqué par `AudioSource` + `AudioSystem`), `memory/atlas/gameplay/input-scripting.md` (le split composant `PlayerInput` + service `InputApi`, calqué par `AudioSource` + `AudioApi`).
...
## 10. Hors périmètre v1 → `memory/atlas/backlog/_index.md#audio`
