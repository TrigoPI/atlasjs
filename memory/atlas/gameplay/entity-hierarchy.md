...
> - `memory/atlas/core/nexus-ecs.md` — l'ECS (entités générationnelles, stores, `query`, command buffer, `onAdd`/`onRemove`).
> - `memory/atlas/gameplay/gameplay-redesign.md` — la source de vérité unique dans Nexus et l'autorité physique par type de body.
> - `memory/atlas/gameplay/scripting-components.md` — les deux niveaux de composants + le modèle de façade `ScriptComponent`.
> - `memory/atlas/core/scheduling.md` — lanes/stages, boucle fixed→update→render.
> - `memory/atlas/rendering/sprites.md` — le `SpriteRenderSystem` qui monte un `SpriteNode` par entité.
...
**Hors-scope (→ `memory/atlas/backlog/`) :**
...
- [x] **Phase 6 — Dino Brawl + docs.** Démo `EcsScene` (attachement natif) + script d'attachement ; mettre à jour `memory/atlas/backlog/` (items reportés) et marquer ce doc « implémenté ».
