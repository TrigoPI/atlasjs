...
| Events d'`Animator` (`memory/atlas/gameplay/animation-events.md`) | Les attaques ne sont pas jouées par un `Animator` — la pose est calculée procéduralement par `AttackTimeline`. Aucun clip d'animation sur lequel s'accrocher. |
